...
/*   Updated: 2022/11/19 18:51:07 by momadani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_cd(t_env *lst, char **av, int ac)
{
	...
	if (ac == 1 || ac == 2)
	{
		...
		if (ac == 1 && chdir(get_key_value(lst, "HOME")))
				ft_error("cd: HOME not set", NULL, NULL, 1);
		if (ac == 2)
		{
			if (!chdir(av[i]))
				replace_pwd("PWD", lst);
			else
				return (ft_error("cd: ", av[i], strerror(errno), 1));
				//ft_error("cd: ", av[i], ": No such file or directory\n", 1);
		}
	}
	else
		ft_error("cd : too many arguments", NULL, NULL, 1);
	return (0);
}
